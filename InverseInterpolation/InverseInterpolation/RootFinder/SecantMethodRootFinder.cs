...
namespace InverseInterpolation.RootFinder
{
    class SecantMethodRootFinder
    {
        ...
        public SecantMethodRootFinder(Func<double, double> function,
        ...
        {
            var rootContainingSegments = SeparateRoots();
            return ClarifyRootsUsingSecantMethod(rootContainingSegments);
        }
        ...
        private List<Segment> SeparateRoots()
        {
            ...
            {
                left = Math.Min(left, rootsFindingSegment.Right);
                var right = left + separationStepLength;
